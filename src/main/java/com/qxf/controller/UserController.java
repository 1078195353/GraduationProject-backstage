...
import com.qxf.entity.ChangePh;
import com.qxf.entity.ChangePw;
...
public class UserController {
    ...
    @MyLog
    public ResultUtil changeUserPw(@RequestBody ChangePw info){
        String msg = "修改失败！";
        User user = userService.getUserByUsername(info.getName());
        user.setPassword(userService.encodePw(info.getNewPw()));
        Integer cnt = userService.updateUser(user);
        if (cnt > 0){
            msg = "修改成功！";
        }
    ...
    @PostMapping("/changePhUrl")
    @MyLog
    public ResultUtil changeUserPw(@RequestBody ChangePh info){
        String msg = "修改失败！";
        User user = userService.getUserByUsername(info.getName());
        user.setPhotoUrl(info.getNewPh());
        Integer cnt = userService.updateUser(user);
        if (cnt > 0){
            msg = "修改成功！";
        }
